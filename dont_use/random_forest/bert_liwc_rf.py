...
class ConcatBertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        new_concat_result_list = []
        for i in range(len(concat_result)):
            concat_list = concat_result[i].tolist()
            concat_list.extend(feature_data[int(index[i].item())])
            new_concat_result_list.append(torch.Tensor(concat_list))

        new_concat_result = torch.stack(new_concat_result_list, dim=0).to('cuda')
        ...
        return modeling_outputs.SequenceClassifierOutput(
            loss=loss,
            logits=output,
            hidden_states=new_concat_result_list,
            attentions=labels.tolist()
        )
...
def evaluate(post_dataloader_val, comment_dataloader_val):
    ...
    with torch.no_grad():
        for p_batch, c_batch in zip(post_dataloader_val, comment_dataloader_val):
            ...
            loss = outputs[0]
            logits = outputs[1]
            eval_rf_list.extend(outputs[2])
            eval_labels_list.extend(outputs[3])
    ...
    for i in range(len(eval_rf_list)):
        eval_rf_list[i] = eval_rf_list[i].tolist()

    pred = rf_regressor.predict(eval_rf_list)
    loss_val_avg = loss_val_total / len(post_dataloader_val)

    predictions = np.concatenate(predictions, axis=0)
    true_vals = np.concatenate(true_vals, axis=0)

    rf_loss = rf_loss_fct(torch.Tensor(pred), torch.Tensor(true_vals))

    for p, t in zip(pred, true_vals):
        print(p, t)
    ...
    # rf pred
    return rf_loss.item(), pred, true_vals
...
for epoch in tqdm(range(1, epochs + 1)):
    ...
    for p_batch, c_batch in zip(post_dataloader_train, comment_dataloader_train):
        ...
        outputs = model(**inputs)
        loss = outputs[0]
        rf_list.extend(outputs[2])
        labels_list.extend(outputs[3])
        ...
        scheduler.step()
        i += 1
    # rf_features = torch.stack(rf_list, dim=0).to('cuda')
    # print(rf_features)
    for i in range(len(rf_list)):
        rf_list[i] = rf_list[i].tolist()
    rf_regressor.fit(rf_list, labels_list)
    ...
    val_loss, pred, true_vals = evaluate(post_dataloader_test, comment_dataloader_test)
    evaluation_result.append([val_loss, pred, torch.tensor(true_vals)])
    print(f'Validation loss: {val_loss}')

    true_vals = evaluation_result[0][2].tolist()
    # predict = sum(evaluation_result[0][1].tolist(), [])
    tqdm.write(f'R^2 score: {r2_score(true_vals, pred)}')

    pred_df = pd.DataFrame(pred)
    pred_df.to_csv(f'../predicting-satisfaction-using-graphs/csv/liwc_rf/batch_{batch_size}_lr_1e-4/epoch_{epoch}_predicted_vals.csv')

    training_result.append([epoch, loss_train_avg, val_loss, r2_score(true_vals, pred)])


fields = ['epoch', 'training_loss', 'validation_loss', 'r^2_score']

with open(f'../predicting-satisfaction-using-graphs/csv/liwc_rf/batch_{batch_size}_lr_1e-4/training_result.csv', 'w', newline='') as f:
...
true_df.to_csv(f'../predicting-satisfaction-using-graphs/csv/liwc_rf/true_vals.csv')