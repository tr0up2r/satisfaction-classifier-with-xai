...
post_df, post_inputs_train, post_input_test, post_IS_labels_train, post_IS_labels_test, post_ES_labels_train, post_ES_labels_test = split_df(post_df)
comment_df, comment_inputs_train, comment_input_test, comment_IS_labels_train, comment_IS_labels_test, comment_ES_labels_train, comment_ES_labels_test = split_df(comment_df)

tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True)


def generate_dataloader(df, IS_labels_train, IS_labels_test, ES_labels_train, ES_labels_test, batch_size):
    ...
    labels_train = []
    for IS, ES in zip(IS_labels_train, ES_labels_train):
        labels_train.append([IS, ES])
    labels_train = torch.tensor([labels_train]).squeeze()

    input_ids_test = encoded_data_test['input_ids']
    attention_masks_test = encoded_data_test['attention_mask']
    labels_test = []
    for IS, ES in zip(IS_labels_test, ES_labels_test):
        labels_test.append([IS, ES])
    labels_test = torch.tensor([labels_test]).squeeze()
...
post_dataloader_train, post_dataloader_test = generate_dataloader(
    post_df, post_IS_labels_train, post_IS_labels_test, post_ES_labels_train, post_ES_labels_test, batch_size)
comment_dataloader_train, comment_dataloader_test = generate_dataloader(
    comment_df, comment_IS_labels_train, comment_IS_labels_test, comment_ES_labels_train, comment_ES_labels_test, batch_size)
...
def train_model_and_save(dataloader_train, dataloader_val, target):
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        IS_true_vals, ES_true_vals, IS_predict, ES_predict = [], [], [], []
        for t, p in zip(true_vals, predict):
            IS_true_vals.append(t[0])
            ES_true_vals.append(t[1])
            IS_predict.append(p[0])
            ES_predict.append(p[1])

        tqdm.write(
            f'IS model R^2 score: {r2_score(IS_true_vals, IS_predict)}\nES model R^2 score: {r2_score(ES_true_vals, ES_predict)}')
        ...
        training_result.append([epoch, loss_train_avg, val_loss, r2_score(IS_true_vals, IS_predict), r2_score(ES_true_vals, ES_predict)])
        torch.save(model.state_dict(), f'../predicting-satisfaction-using-graphs/csv/IS_ES_regressor/model/{target}/epoch_{epoch}.model')

    fields = ['epoch', 'training_loss', 'validation_loss', 'IS_r^2_score', 'ES_r^2_score']
    ...
    true_df.to_csv(f'../predicting-satisfaction-using-graphs/csv/{target}_IS_ES_true_vals.csv')


train_model_and_save(comment_dataloader_train, comment_dataloader_test, 'comment')
train_model_and_save(post_dataloader_train, post_dataloader_test, 'post')
