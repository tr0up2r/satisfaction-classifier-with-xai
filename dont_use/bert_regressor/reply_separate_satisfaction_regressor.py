...
for reply, content, body, satisfaction in zip(reply_contents, post_contents, comment_bodies, satisfactions):
    if content != '[deleted]' and content != '[removed]' and body != '[deleted]' and body != '[removed]':
        r_sentences = list(nlp(reply).sents)
        for r in r_sentences:
            reply_data.append([str(r), satisfaction])
...
for epoch in tqdm(range(1, epochs + 1)):
    ...
    pred_df.to_csv(f'../predicting-satisfaction-using-graphs/csv/reply_separate_regressor/batch_{batch_size}_lr_1e-5/epoch_{epoch}_predicted_vals.csv')
...
with open(f'../predicting-satisfaction-using-graphs/csv/reply_separate_regressor/batch_{batch_size}_lr_1e-5/training_result.csv', 'w', newline='') as f:
