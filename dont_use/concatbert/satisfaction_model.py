import pandas as pd
import numpy as np
# import IS_ES_seperate_regressor
...
class ConcatWeightBertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert_post_IS = BertForSequenceClassification.from_pretrained('bert-base-uncased', num_labels=3)
        self.bert_post_ES = BertForSequenceClassification.from_pretrained('bert-base-uncased', num_labels=3)
        self.bert_comment_IS = BertForSequenceClassification.from_pretrained('bert-base-uncased', num_labels=3)
        self.bert_comment_ES = BertForSequenceClassification.from_pretrained('bert-base-uncased', num_labels=3)
        self.classifier = nn.Linear(config.hidden_size*2, 768)
        self.classifier2 = nn.Linear(config.hidden_size*2, 768)
        self.classifier3 = nn.Linear(config.hidden_size+4, 1)
    ...
    def forward(
        ...
        self.bert_post_IS.load_state_dict(
            torch.load(f'../predicting-satisfaction-using-graphs/csv/IS_ES_classifier/model/post_IS/epoch_5.model',
                       map_location='cuda:0'))
        self.bert_post_ES.load_state_dict(
            torch.load(f'../predicting-satisfaction-using-graphs/csv/IS_ES_classifier/model/post_ES/epoch_3.model',
                       map_location='cuda:0'))
        self.bert_comment_IS.load_state_dict(
            torch.load(f'../predicting-satisfaction-using-graphs/csv/IS_ES_classifier/model/comment_IS/epoch_3.model',
                       map_location='cuda:0'))
        self.bert_comment_ES.load_state_dict(
            torch.load(f'../predicting-satisfaction-using-graphs/csv/IS_ES_classifier/model/comment_ES/epoch_7.model',
                       map_location='cuda:0'))

        def get_support_score(model, ids):
            score = model(ids)
            score_label = np.argmax(score[0].cpu().detach().numpy(), axis=1)
            score_label_list = []
            for i in range(len(score_label)):
                if score_label[i] == 0:
                    score_label_list.append([1])
                elif score_label[i] == 1:
                    score_label_list.append([2])
                else:
                    score_label_list.append([3])
            score_label = torch.Tensor(score_label_list).to('cuda')

            return score_label

        post_IS_label = get_support_score(self.bert_post_IS, post_input_ids)
        post_ES_label = get_support_score(self.bert_post_ES, post_input_ids)
        comment_IS_label = get_support_score(self.bert_comment_IS, comment_input_ids)
        comment_ES_label = get_support_score(self.bert_comment_ES, comment_input_ids)
        ...
        output = self.classifier3(torch.cat((weight_result, post_IS_label, post_ES_label, comment_IS_label, comment_ES_label), dim=1))
