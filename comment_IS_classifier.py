...
df = df.drop(df[(df.comment_body == '[removed]') | (df.comment_body == '[deleted]')].index)
IS_count = df['comment_IS'].value_counts()
# print(f'emt_count :\n{emt_count}\n')

possible_labels = df.comment_IS.unique()
...
df['label'] = df.comment_IS.replace(label_dict)
# print(df, '\n')
...
encoded_data_train = tokenizer.batch_encode_plus(
    df[df.data_type == 'train'].comment_body.values,
...
encoded_data_val = tokenizer.batch_encode_plus(
    df[df.data_type == 'val'].comment_body.values,
    add_special_tokens=True,
    return_attention_mask=True,
    pad_to_max_length=True,
    max_length=512,
...
for epoch in tqdm(range(1, epochs + 1)):
    ...
    for batch in progress_bar:
        ...
        i += 1
    torch.save(model.state_dict(), f'../predicting-satisfaction-using-graphs/csv/IS_ES_classifier/model/comment_IS/epoch_{epoch}.model')
    tqdm.write(f'\nEpoch {epoch}')
...
with open(
        f'../predicting-satisfaction-using-graphs/csv/IS_ES_classifier/batch_{batch_size}_lr_1e-5/comment_IS/training_result.csv',
        'w', newline='') as f:
