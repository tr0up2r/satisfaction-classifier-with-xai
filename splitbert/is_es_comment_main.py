...
if __name__ == "__main__":
    ...
    for mode in modes:
        ...
        for post, comment, label in zip(post_sequences, comment_sequences, is_score):
            ...
            data.append([i, comment, label, 1.5])
            i += 1

        print(max_post, max_comment)
        max_count = max(max_post, max_comment)

        columns = ['index', 'comment_contents', 'label', 'score']
        ...
        dataset_train = conduct_input_ids_and_attention_masks(tokenizer, [train_sample_df.comment_contents.values],
                                                                    train_sample_df.label.values,
                                                                    train_sample_df.score.values,
                                                              train_sample_df.index.values, max_count, 'reply')

        dataset_val = conduct_input_ids_and_attention_masks(tokenizer, [val_df.comment_contents.values],
        ...
        train(model, device, dataset_train, dataset_val, labels, 'reply', path, ['seg'])
