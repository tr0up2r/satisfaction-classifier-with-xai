...
if __name__ == '__main__':
    ...
    # for linux
    df = pd.read_csv(path + '/csv/dataset/df_for_contrastive_learner_train_50.csv',
                     encoding='UTF-8')
    test_df = pd.read_csv(
        path + '/csv/dataset/df_for_contrastive_learner_test_remain.csv',
        encoding='UTF-8')
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        tsne_df.to_csv(path + f'/csv/contrastive_learner/epoch_{epoch}_result.csv')
        print(model.splitbert.state_dict())
        torch.save(model.splitbert.state_dict(),
                   path + f'/model/contrastive_learner/epoch_{epoch}_model.pt')

    fields = ['epoch', 'training_loss', 'validation_loss', 'r^2_score']

    with open(
            path + '/csv/contrastive_learner/training_result.csv',
            'w', newline='') as f:
