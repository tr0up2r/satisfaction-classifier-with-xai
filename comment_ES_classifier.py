...
ES_count = df['comment_ES'].value_counts()
# print(f'emt_count :\n{emt_count}\n')

possible_labels = df.comment_ES.unique()
...
df['label'] = df.comment_ES.replace(label_dict)
# print(df, '\n')
...
for epoch in tqdm(range(1, epochs + 1)):
    ...
    for batch in progress_bar:
        ...
        i += 1
    torch.save(model.state_dict(), f'../predicting-satisfaction-using-graphs/csv/IS_ES_classifier/model/comment_ES/epoch_{epoch}.model')
    tqdm.write(f'\nEpoch {epoch}')
...
with open(
        f'../predicting-satisfaction-using-graphs/csv/IS_ES_classifier/batch_{batch_size}_lr_1e-5/comment_ES/training_result.csv',
        'w', newline='') as f:
