...
class SplitBertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        batch_embeddings = torch.empty(size=(len(input_ids), 1, max_sentences, embedding_size), requires_grad=True).to('cuda')
        for i in range(len(input_ids)):
            ...
            # print(input_ids[i][0:sentence_count[i]].shape)
            ...
            # model_output = model_output.unsqueeze(0).swapaxes(0, 1)
        ...
        encoder_outputs = torch.empty(size=(len(input_ids), embedding_size), requires_grad=True).to('cuda')
...
encoded_input = tokenizer(train_sample_df.contents.values[0], padding=True, truncation=True, max_length=128, return_tensors='pt')
print(encoded_input)
