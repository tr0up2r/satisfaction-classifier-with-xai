...
IS_count = df['post_IS'].value_counts()
# print(f'emt_count :\n{emt_count}\n')

possible_labels = df.post_IS.unique()
...
df['label'] = df.post_IS.replace(label_dict)
# print(df, '\n')
...
for epoch in tqdm(range(1, epochs + 1)):
    ...
    for batch in progress_bar:
        ...
        i += 1
    torch.save(model.state_dict(), f'../predicting-satisfaction-using-graphs/csv/IS_ES_classifier/model/post_IS/epoch_{epoch}.model')
    tqdm.write(f'\nEpoch {epoch}')
...
with open(
        f'../predicting-satisfaction-using-graphs/csv/IS_ES_classifier/batch_{batch_size}_lr_1e-5/post_IS/training_result.csv',
        'w', newline='') as f:
