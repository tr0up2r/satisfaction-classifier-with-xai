...
class ContrastiveLearner(nn.Module):
    def __init__(self):
        ...
        self.fc_layer1 = nn.Linear(embedding_size*3, embedding_size)
        self.fc_layer2 = nn.Linear(embedding_size, num_labels)
    ...
    def forward(self,
        ...
        outputs = self.fc_layer1(outputs)
...
def make_train_test_df(df):
    ...
    for i1, p1, c1, l1, lc1, s1, i2, p2, c2, l2, lc2, s2 in zip(index1, post_sequences1, comment_sequences1, label1,
        ...
        # add 4 for one-hot labeling
        data.append([i1, p1, c1, l1, lc1, i2, p2, c2, l2, lc2, lc1-lc2+4])
...
def conduct_input_ids_and_attention_masks(str_values, label_values1, label_values2, diff_values,
    ...
    diffs = torch.tensor(diff_values.astype(int))
...
def evaluate(dataloader_val):
    ...
    for batch in dataloader_val:
        ...
        one_hot_labels = torch.nn.functional.one_hot(batch[14], num_classes=num_labels)
        ...
        inputs = {'post_input_ids1': batch[0],
                  ...
                  'labels1': batch[12],
                  'labels2': batch[13],
                  'diffs': one_hot_labels.type(torch.float),
                  'indexes1': batch[15],
                  'indexes2': batch[16]
                  }
...
if __name__ == '__main__':
    ...
    # for linux
    df = pd.read_csv('../predicting-satisfaction-using-graphs/csv/dataset/df_for_contrastive_learner_train_49500.csv',
                     encoding='UTF-8')
    test_df = pd.read_csv(
        '../predicting-satisfaction-using-graphs/csv/dataset/df_for_contrastive_learner_test_9900.csv',
        encoding='UTF-8')
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        tsne_df.to_csv(f'../predicting-satisfaction-using-graphs/csv/contrastive_learner/classification/epoch_{epoch}_result.csv')
        torch.save(model.splitbert.state_dict(),
                   f'../predicting-satisfaction-using-graphs/model/contrastive_learner/classification/epoch_{epoch}_model.pt')

    fields = ['epoch', 'training_loss', 'validation_loss', 'f1_score_macro', 'f1_score_micro']

    with open(
            f'../predicting-satisfaction-using-graphs/csv/contrastive_learner/classification/training_result.csv',
            'w', newline='') as f:
